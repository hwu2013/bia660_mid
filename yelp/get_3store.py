...
#echo ${#secondbig[@]}
#echo ${#thirdbig[@]}
city=["New York","Newark","Jersey City","Paterson"]


#        ./main.py_onestate --location="$i" --term="century 21"  --maxrow=1000 --writeto="./output_1/${i}.log"

mxr = "--maxrow=1000"
ter = ["--term='century 21'", "--term='coldwell banker'","--term='RE/MAX'","--term='Keller Williams'"]
store = ["stores/c21","stores/cb","stores/rm","stores/kw"]


for t,s in zip(ter,store):
    for c in city:
        loc = "--location="+c
        wri ="--writeto="+s+"/"+c+".txt"
        p1 = subprocess.Popen(["./main.py_onestate",loc,t,mxr,wri], stdout=subprocess.PIPE)
        time.sleep(1)
